...
from ge.models import Dataset, KeyLink, WordMap, KeyWord, WFControl
...
def MapRedProcess(v_ds):
    ...
    # Start run dataset per interation
    for qs in qs_queryset:
        print('Starting the dataset: %s' % qs.database)

        # Check control proccess
        try:
            qs_wfc = WFControl.objects.get(dataset_id = qs.id, chk_collect=True, chk_prepare=True, chk_commute=True, chk_mapreduce=False)
        except ObjectDoesNotExist:
        ...
        DFWORD = pd.DataFrame(list(KeyWord.objects.values('word','keyge','keyge_id__keyge').filter(status=True)))
 
        # Search through Word what is the Keyge ID
        if DFWORD.empty:
            print("No data on Keyword")
            DFR["keyge1_id"] = ""
            DFR["keyge2_id"] = ""
        else:
            DFR["keyge1_id"] = DFR.set_index("word1").index.map(DFWORD.set_index("word")["keyge"])
            DFR["keyge2_id"] = DFR.set_index("word2").index.map(DFWORD.set_index("word")["keyge"])

        #Normalize DataFrame to load on GE tables
        ...
        # v_schema = 0 will record all words on WordMap to analysis
        if v_schema == 0:
        ...
        # convert keyge_ig to load table
